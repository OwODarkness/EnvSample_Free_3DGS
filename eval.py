...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, save_ims):
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        t1 = time.time()
        g_buffer = render(view, gaussians, pipeline, background, mode="render")
        pbr_shading(g_buffer, gaussians.env, gaussians.nerual_specular)
    ...
    dump_path = os.path.join(model_path, 'fps.txt')
